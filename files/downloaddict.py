...
"""
Automatically downloads external files.
"""
...
class DictionaryDownloader:
    ...
    def __init__(self, targetName=None, softLink=False):
...
def main():
    # Get Dictionaries
    optionList = [o for o in sys.argv[1:] if o.startswith('-')]
    nameList = [o for o in sys.argv[1:] if not o.startswith('-')]

    options = {}
    if '--softLink' in optionList:
        options['softLink'] = True
    for name in optionList:
        if name.startswith('--targetName'):
            _, targetName = name.split('=', 1)
            options['targetName'] = targetName.strip()

    if 'targetName' in options and len(nameList) > 1:
        raise ValueError(
            "Option 'targetName' can only be specified for a single dictionary")

    for dictionary in nameList:
        print "Downloading %s" % dictionary
        downloaderClass = CLASS_DICT[dictionary]
        downloaderClass(**options).download()
